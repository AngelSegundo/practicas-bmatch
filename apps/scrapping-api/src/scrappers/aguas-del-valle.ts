...
const MAIN_URL    = "https://www.aguasdelvalle.cl/personas/inicio";
const URL_API     = 'https://ov.esval.cl/api-ov/api'



export interface aguasDelValleScrapInput {
  clientId: string;
}

export const aguasDelValleScrap = async ({
  clientId,
}: aguasDelValleScrapInput): Promise<MiningDate[]> => {
    ...
    await page.type("#contrasena", "Nubisk23");
    ...
    /* const getToken = await new Promise<string>(async(resolve, reject) => {
    ...
    //const token = getToken.split("auth=")[1] as string
