...
import {
  CommunityDTOSchema,
  ServiceConnectionDTOSchema,
  ServiceDTOSchema,
  ServiceReadingDTOSchema,
  SponsorDTOSchema,
  UserDTOSchema,
...
const tableSchemaMap = {
  readings: ServiceReadingDTOSchema.merge(BigQueryRecord),
  users: UserDTOSchema.merge(BigQueryRecord),
  communities: CommunityDTOSchema.merge(BigQueryRecord),
  connections: ServiceConnectionDTOSchema.merge(BigQueryRecord),
  services: ServiceDTOSchema.merge(BigQueryRecord),
...
const main = async () => {
  ...
  await firestoreToBigQuery(["readings"]);
  await firestoreToBigQuery(["users"]);
  await firestoreToBigQuery(["communities"]);
  await firestoreToBigQuery(["services"]);
  await firestoreToBigQuery(["sponsors"]);
  await firestoreToBigQuery(["connections"]);
};
